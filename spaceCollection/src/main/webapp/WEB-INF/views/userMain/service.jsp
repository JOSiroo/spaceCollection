...
제1장 총 칙
제1조 (목적)
이 약관(이하 “약관”이라 합니다)은 ㈜스페이스클라우드(이하 “회사”라 합니다)와 이용 고객(이하 “회원”이라 합니다)간에 회사가 제공하는 서비스(이하 “서비스”라 합니다)를 이용함에 있어 회원과 회사간의 권리, 의무 및 책임사항, 이용조건 및 절차 등 기본적인 사항을 규정함을 목적으로 합니다.

제2조 (용어의 정의)
본 약관에서 사용되는 용어의 정의는 다음과 같습니다.

(1) “서비스” : 회사가 제공하는 여가(숙박∙레저∙티켓∙클래스 등으로 이에 한정되지 않음) 관련 상품∙용역∙서비스 등(이하 “상품등”이라 합니다)에 대한 예약∙구매∙정보제공∙추천 등을 위한 온라인 플랫폼 서비스를 의미합니다. 서비스는 구현되는 장치나 단말기(PC, TV, 휴대형 단말기 등의 각종 유무선 장치를 포함하며 이에 한정되지 않음)와 상관 없이 스페이스클라우드 및 스페이스클라우드 관련 웹(Web)∙앱(App) 등의 제반 서비스를 의미하며, 회사가 공개한 API를 이용하여 제3자가 개발 또는 구축한 프로그램이나 서비스를 통하여 이용자에게 제공되는 경우를 포함합니다. 서비스는 여가 문화가 변하고 기술이 발전함에 따라 지속적으로 함께 변화∙성장해 나갑니다.
...
(7) "스페이스클라우드 코인" : 포인트로 교환하여 사용할 수 있는 수치화 된 가상의 데이터를 말합니다. 구체적인 이용 방법, 그 명칭 등은 회사의 정책에 따라 달라질 수 있습니다.
...
제19조 (스페이스클라우드코인)
① 스페이스클라우드코인은 회사의 정책에 따라 회원에게 제공되며, 코인별 적립기준, 사용방법, 사용기한 및 제한에 관한 사항은 회사 정책에 따라 별도 공지합니다.
② 스페이스클라우드코인은 사용기한 동안 사용되지 않거나 회원 탈퇴 또는 자격상실 사유가 발생한 경우 자동으로 소멸됩니다. 회원 탈퇴 내지 자격상실로 스페이스클라우드코인이 소멸된 경우 재 가입하더라도 소멸된 스페이스클라우드코인은 복구되지 않습니다.
③ 회원은 회사가 별도로 정한 경우를 제외하고는 스페이스클라우드코인을 제3자에게 양도할 수 없습니다. 만일 회원이 회사가 승인하지 않은 방법으로 스페이스클라우드코인을 획득/이용한 사실이 확인될 경우 회사는 회원 자격을 정지 또는 해지할 수 있습니다.
④ 스페이스클라우드코인 관련 회사의 정책은 회사의 영업정책에 따라 변동될 수 있습니다. 회원에게 불리한 변경인 경우에는 제3조의 규정에 따라 공지 또는 통지하며 서비스 계속 이용시 동의한 것으로 간주됩니다.
...
③ 회원은 서비스 이용과 관련하여 다음 각 호의 행위를 하여서는 안됩니다.
  ...
  (12) 회사의 웹 ∙ 앱과 서버 간의 전송 내용 등을 확인하거나 소스 코드 등의 추출을 시도하기 위해 패킷 캡처 및 이와 유사 ∙ 동일한 기능의 프로그램 등을 사용하여 회사의 웹 ∙ 앱 ∙ 서버에 접근하거나 해당 프로그램 등을 통하여 알게 된 소스 코드(URL 포함) 및 명령 구문 등을 사용하여 회사의 웹 ∙ 앱 ∙ 서버에 접근하는 행위
  (13) 회사의 명시적인 사전 서면 허가 없이 데이터 등을 추출하기 위하여 로봇(봇), 크롤러, 스파이더, 스크레이퍼, 매크로 프로그램 또는 기타 자동화 수단이나 수동 프로세스를 이용하여 회사의 웹 ∙ 앱 ∙ 서버에 접근하거나 회사 서비스에 포함된 콘텐츠 및 정보를 모니터링, 복제, 수집, 확인, 정리, 사용하는 등의 행위
  (14) 서비스와 관련된 설비의 오동작이나 정보 등의 파괴 및 혼란을 유발시키는 컴퓨터 바이러스, 기타 다른 컴퓨터 코드, 파일, 프로그램 자료를 등록 또는 유포하는 행위
  (15) 회사가 제공하는 소프트웨어나 하드웨어 등을 개작하거나 해독, 리버스 엔지니어링, 디컴파일, 디스어셈블 및 기타 일체의 가공 행위를 통하여 서비스를 복제, 분해 또는 모방 기타 변형하는 행위 및 이를 목적으로 또는 이를 통하여 알게 된 소스코드(URL 포함) 및 명령 구문 등을 사용하여 회사의 웹 ∙ 앱 ∙ 서버에 접근하는 행위
  (16) 자동 접속 프로그램 등을 사용하는 등 정상적인 용법과 다른 방법으로 서비스를 이용하여 회사의 서버에 부하를 일으켜 회사의 정상적인 서비스를 방해하는 행위
  (17) 회사의 웹 ∙ 앱 ∙ 서버에서 로봇 배제 헤더의 제한 사항을 위반하거나 회사의 웹 ∙ 앱 ∙ 서버 및 서비스에 대한 접근을 방지 또는 제한하기 위해 채택되는 기타 조치를 우회 또는 회피하는 일체의 행위(예: IP를 지속적으로 바꿔가며 접속하는 행위, Captcha를 외부 솔루션 등을 통해 우회하거나 무력화하는 행위 등) 및 이를 통하여 회사의 웹 ∙ 앱 ∙ 서버에 접근하는 행위
  (18) 회사의 사전 서면 승인 없이 회사의 웹, 앱, 서비스, 콘텐츠, 회사의 브랜딩 또는 페이지 레이아웃이나 디자인 등을 사용, 복제, 전시, 미러링, 프레이밍, 딥링크 하는 행위
  (19) 해킹, 비밀번호 마이닝 또는 기타 수단을 통해 회사의 웹 ∙ 앱 ∙ 서버, 기타 관련 웹사이트, 다른 계정, 컴퓨터 시스템 및 네트워크 등에 대한 승인되지 않은 접속 권한을 얻기 위해 시도하는 행위 및 이를 통하여 회사의 웹 ∙ 앱 ∙ 서버 등에 접근하는 행위
  (20) 회사가 제공하는 서비스 및 콘텐츠∙정보를 상업적이거나 경쟁적인 목적, 회사의 사업 현황을 파악하기 위한 목적, 불법적이거나 비합법적인 목적, 또는 본 이용 약관에서 금지하는 목적 등으로 사용하기 위하여 회사의 웹 ∙ 앱 ∙ 서버에 접근하는 일체의 행위
